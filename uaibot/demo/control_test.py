...
def _control_demo_davinci(arm=0):
    ...
    wall1 = Box(name="wall1", htm=Utils.trn([1.2e1, 0.5, 0.7]), width=0.05, depth=0.6, height=1.4e-2,
                mesh_material=material_wall)
    wall2 = Box(name="wall2", htm=Utils.trn([1.2e1, 1.5, 0.7]), width=0.05, depth=0.6, height=1.4e-2,
                mesh_material=material_wall)
    wall3 = Box(name="wall3", htm=Utils.trn([1.2e1, 1, 0.25]), width=0.05, depth=0.4, height=0.5e-2,
                mesh_material=material_wall)
    wall4 = Box(name="wall4", htm=Utils.trn([1.2e1, 1, 1.15]), width=0.05, depth=0.4, height=0.5e-2,
                mesh_material=material_wall)
    ...
    def dist_computation(q, old_struct, h):
        ...
        # gradD_list = np.r_[dist_wall_1.get_closest_item().jac_distance, dist_wall_2.get_closest_item().jac_distance,
        #                    dist_wall_3.get_closest_item().jac_distance, dist_wall_4.get_closest_item().jac_distance]
        gradD_list = np.block(
        ...
        # dist_vect = np.block([[dist_wall_1.get_closest_item().distance], [dist_wall_2.get_closest_item().distance], [
        #                      dist_wall_3.get_closest_item().distance], [dist_wall_4.get_closest_item().distance]])

        return dist_vect, gradD_list, struct

    # Target pose definition
    pose_tg = []
    pose_tg.append(Utils.trn([0.3, 1, 1.2]) @ Utils.rotx(-np.pi/2))
    ...
    for k in range(len(pose_tg)):
        ...
        while not converged and not error_qp and not iteration_end:
            ...
            #print(gradD_list.shape, dist_vect.shape, qdot.shape)
