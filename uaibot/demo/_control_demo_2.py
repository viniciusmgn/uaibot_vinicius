...
def _control_demo_2():
    ...
    for i in range(round(time_max_1 / dt)):
        ...
        hist_q_a.append(robot_a.q.reshape((6,)))
        hist_qdot_a.append(u[0:6].reshape((6,)))
        hist_q_b.append(robot_b.q.reshape((6,)))
        hist_qdot_b.append(u[6:12].reshape((6,)))
        hist_error_rel_pos.append(r[0:3].reshape((3,)))
    ...
    Utils.plot(hist_time, np.transpose(hist_error_rel_pos), "", "Time (s)", "Relative position error (m)",
               ['x', 'y', 'z'])
    Utils.plot(hist_time, np.transpose(hist_error_rel_ori), "", "Time (s)", "Relative orientation error (degrees)",
               ['x', 'y', 'z'])
